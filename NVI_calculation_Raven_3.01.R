...
InputSelFileName <- "SHAL-KODAI-YO__-0001__notes_Faizee"
InputCorrFileName <- "SHAL_YO__-0001_BatchCorrOutput"

Selections <- paste(InputSelFileName,".txt", sep  = "")
BatchCorrInp <- paste(InputCorrFileName,".txt", sep  = "")
...
Notes_Selections <-as.data.frame(read_tsv(Selections, col_names = T))
drop <- c("Complex Note?")
Notes_Selections = Notes_Selections[,!(names(Notes_Selections)%in% drop)]
...
BatchCorrOutput <- read_tsv(BatchCorrInp, col_names = T, skip = 2)
...
NVIoptxtfile = paste(substr(InputCorrFileName, 1,15), "NVI_Output.txt")
write.table(Song_Selections_Output, file = NVIoptxtfile, sep = "\t", quote = F,row.names = FALSE)

#Export NVI output as a csv file
NVIopcsvfile = paste(substr(InputCorrFileName, 1,15), "NVI_Output.csv")
write.csv(NVI_output, NVIopcsvfile)
